...
class TransactionRepo {
  ...
  Future<ApiResponse> getBonusTransaction(int? id) async {
    try {
      print("bonustransactionsUri ${id}");
      final Response response = await dioClient!.get(
        '${AppConstants.bonustransactionsUri}',
        // data: {"user_id": "$id"}
      );
      print("bonustransactionsUri ${response.data}");
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> getAddmoney(String tId, String amount) async {
    try {
      Map data = {"transaction_id": tId, "amount": amount};
      print('Request $data');
      final Response response = await dioClient!.post(
          '${AppConstants.addMoney}',
          data: {"transaction_id": tId, "amount": amount});
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  // Future<ApiResponse> getFundmoney(String tId, String amount) async {
  //   try {
  //     Map data = {"transaction_id": tId, "amount": amount};
  //     print('Request $data');
  //     final Response response = await dioClient!.post(
  //         '${AppConstants.addFundMoney}',
  //         data: {"transaction_id": tId, "amount": amount});
  //     return ApiResponse.withSuccess(response);
  //   } catch (e) {
  //     return ApiResponse.withError(ApiErrorHandler.getMessage(e));
  ...
  Future<ApiResponse> initget_wallet_transaction() async {
    try {
      final Response response =
          await dioClient!.get('${AppConstants.getWalletTransaction}');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
