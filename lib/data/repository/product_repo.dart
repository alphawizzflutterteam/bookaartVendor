...
class ProductRepo {
  ...
  Future<ApiResponse> getSellerProductList(
    ...
    try {
      print(
          "idididi $sellerId/all-products?limit=10&&offset=$offset&search=$search");
  ...
  Future<ApiResponse> getPosProductList(int offset) async {
    try {
      final response = await dioClient!
          .get('${AppConstants.posProductList}?limit=10&&offset=$offset');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> updateProductStatus(int status, int id) async {
    try {
      final response = await dioClient!
          .get('${AppConstants.updateProductStatus}?status=$status&id=$id');
      print("ksehfiose response $response");
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
