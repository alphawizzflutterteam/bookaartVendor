...
class ProductProvider extends ChangeNotifier {
  ...
  Future<void> updateProductStatusPro(int status, int id) async {
    _isLoading = true;
    ApiResponse apiResponse =
        await productRepo!.updateProductStatus(status, id);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      var result = apiResponse.response;
      var finalResult = jsonDecode(result.toString());
      if (finalResult['success'] != null) {
        Fluttertoast.showToast(msg: finalResult['success']);
