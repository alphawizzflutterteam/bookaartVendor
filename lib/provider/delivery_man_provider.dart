...
class DeliveryManProvider extends ChangeNotifier {
  ...
  Future showOptions(BuildContext context, isProfile, isRemove) async {
        ...
        actions: [
          CupertinoActionSheetAction(
            ...
            onPressed: () {
              ...
              pickImageFromGallery(isProfile, isRemove);
            },
          ),
          CupertinoActionSheetAction(
            ...
            onPressed: () {
              Navigator.of(context).pop();
              pickImageFromCamera(isProfile, isRemove);
            },
          ),
        ],
      ),
  ...
  void pickImageFromGallery(bool isProfile, bool isRemove) async {
    if (isRemove) {
      _profileImage = null;
      _identityImages = [];
    } else {
  ...
  void pickImageFromCamera(bool isProfile, bool isRemove) async {
    if (isRemove) {
      _profileImage = null;
      _identityImages = [];
    } else {
  ...
  Future<void> getTopDeliveryManList(BuildContext context,
    ...
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      final topDeliveryMan = TopDeliveryMan.fromJson(apiResponse.response!.data);

      if (topDeliveryMan.deliveryMan != null && topDeliveryMan.deliveryMan!.isNotEmpty) {
        _topDeliveryManList ??= []; // initialize if null
        _topDeliveryManList!.addAll(topDeliveryMan.deliveryMan!);
      }
    }
    else {
      ApiChecker.checkApi(apiResponse);
  ...
  Future<void> deleteDeliveryManNew(
      BuildContext context, int? deliveryManId) async {
    ApiResponse apiResponse;
    apiResponse = await deliveryManRepo!.deleteDeliveryMan(deliveryManId);
    if (apiResponse.response != null &&
