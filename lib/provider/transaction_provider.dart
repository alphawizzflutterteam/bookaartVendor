...
import 'package:sixvalley_vendor_app/data/model/response/get_wallet_model.dart';
...
import '../data/model/response/bonus_transaction.dart';
...
class TransactionProvider extends ChangeNotifier {
  ...
  List<BonusTransaction>? _bonustransactionList;
  late List<TransactionModel> _allTransactionList;
  List<TransactionModel>? get transactionList => _transactionList;
  List<BonusTransaction>? get bonustransactionList => _bonustransactionList;
  ...
  List<int?> _yearIds = [];
  List<WalletData?> _walletList = [];
  List<int?> _monthIds = [];
  List<WalletData?> get walletList => _walletList;
  ...
  Future<void> getBonusList(BuildContext context, int? id) async {
    ApiResponse apiResponse = await transactionRepo!.getBonusTransaction(id);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      _bonustransactionList = [];
      _bonustransactionList = [];
      apiResponse.response!.data.forEach((transaction) {
        _bonustransactionList!.add(BonusTransaction.fromJson(transaction));
        // _bonustransactionList!.add(BonusTransaction.fromJson(transaction));
      });
    } else {
      ApiChecker.checkApi(apiResponse);
  ...
  Future<void> addMoney(BuildContext context, tid, amount) async {
    amount = amount / 100;
    ApiResponse apiResponse =
        await transactionRepo!.getAddmoney(tid, amount.toString());
    // await transactionRepo!.getTransactionList(tid, amount);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      // return true;
      // _transactionList = [];
  ...
  // Future<void> addFundMoney(BuildContext context, tid, amount) async {
  //   amount = amount / 100;
  //   ApiResponse apiResponse =
  //       await transactionRepo!.getFundmoney(tid, amount.toString());
  //   // await transactionRepo!.getTransactionList(tid, amount);
  //   if (apiResponse.response != null &&
  //       apiResponse.response!.statusCode == 200) {
  //     // return true;
  ...
  void initget_wallet_transaction() async {
    _walletList = [];
    //WalletModel

    ApiResponse apiResponse =
        await transactionRepo!.initget_wallet_transaction();
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      _walletList = [];

      print('object Ankush ${apiResponse.response!.data}');
      _walletList = WalletModel.fromJson(apiResponse.response!.data).data!;
